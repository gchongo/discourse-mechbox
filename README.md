...
**Phase 0.5.3（增量恢复中）**：论坛稳定运行；`/mechbox` 支持硬刷新；`metadata` 含工具目录。详见下方版本记录。
...
| `phase0_5_3` | 2026-07-11 | 恢复 `tool_catalog.rb`；`metadata` 返回工具目录；新增 `mechbox_page_controller` 修复 `/mechbox` 硬刷新 404 | `GET /mechbox` 刷新不 404；`metadata.builtin_tools` 含 5 项 |
