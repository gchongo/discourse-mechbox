...
**安全骨架模式（safe_skeleton）**：论坛可稳定运行；侧边栏有 MechBox 入口，`/mechbox` 为占位页；`status`/`metadata` API 可用，其余端点返回 `501`。业务代码在 `archived/`，按 [docs/PHASED_ROADMAP.md](docs/PHASED_ROADMAP.md) 小步恢复。502 事故原因见 [docs/BOOT_FAILURES.md](docs/BOOT_FAILURES.md)。
