...
**Phase 1.3（待生产验证）**：首个单工具 `gear_ratio` 端到端；`POST /mechbox/api/calculate` 可用；工具页复用稳定单路由 `/mechbox?tool_id=gear_ratio`。
...
| `phase0_5_4b` | 2026-07-11 | 删除独立 `feature_gate.rb`，将同一门禁方法直接放入 `BaseController`，避免额外 autoload/include 链 | 生产验证通过 |
| `phase0_5_5` | 2026-07-11 | 恢复独立 `MetadataBuilder` 与 `MetadataController`；路由改为 `metadata#show`；暂以空数组/对象返回模板、收藏和偏好 | 生产验证通过 |
| `phase0_5_6` | 2026-07-11 | 恢复无数据库依赖的 `ToolsController`；启用 `/tools` 与 `/tools/:tool_id`；`tools` capability 改为 enabled | 生产验证通过 |
| `phase0_5_7` | 2026-07-11 | `/mechbox` 路由加载 metadata；页面展示 5 个内置工具及延期设计链；保持单路由且无动态 helper | 生产验证通过 |
| `phase0_5_8` | 2026-07-11 | 恢复 `user_preferences.rb`；metadata 返回 `preferences` 与 `settings.effective_unit_system` | 生产验证通过 |
| `phase0_5_9` | 2026-07-11 | 恢复 `guardian_extension.rb` + `Guardian.prepend`；API/页面校验 `can_use_mechbox?`；metadata 返回 `can_manage_templates` | 生产验证通过 |
| `phase1_1` | 2026-07-12 | 恢复 `calculator_registry.rb`、`formula_evaluator.rb`；启用 `calculate` / `calculate_validate` capability | 待验证 |
| `phase1_2` | 2026-07-12 | 恢复 `calculation_runner.rb`、`calculations_controller.rb`；路由 `POST /calculate` | 待验证 |
| `phase1_3` | 2026-07-12 | 单路由 `/mechbox?tool_id=gear_ratio`；首页仅 `gear_ratio` 可点击；工具页表单 + 计算结果；旧 `/mechbox/tools/:tool_id` 服务端跳转 | 待验证：齿轮比可算；首页不 502 |
