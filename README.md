...
**Phase 0.5.5（待生产验证）**：独立 metadata builder/controller 已恢复，不依赖 Guardian、用户偏好或数据库模型。页面仍为占位内容，工具目录 UI 计划在 Phase 0.5.7 接入。
...
| `phase0_5_5` | 2026-07-11 | 恢复独立 `MetadataBuilder` 与 `MetadataController`；路由改为 `metadata#show`；暂以空数组/对象返回模板、收藏和偏好 | 待验证：metadata 返回 `mode: phase0_5_5` 和 5 个内置工具 |
| `phase0_5_6` | — | 计划：恢复 `tools_controller` 与工具目录端点 | 待做 |
