...
**Phase 0.5.8（待生产验证）**：metadata 已包含用户偏好（`preferences`）与 `effective_unit_system`；偏好读写 API 仍待后续阶段。
...
| `phase0_5_8` | 2026-07-11 | 恢复 `user_preferences.rb`；metadata 返回 `preferences` 与 `settings.effective_unit_system` | 待验证：默认 `favorite_layout: grid`；自定义偏好可回读 |
| `phase0_5_9` | — | 计划：恢复 `guardian_extension` 与权限控制 | 待做 |
