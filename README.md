...
**Phase 0.5.9（待生产验证）**：平台 API 阶段收尾；已恢复 `Guardian` 权限扩展，API 与 `/mechbox` 页面均校验 `mechbox_allowed_groups`。
...
| `phase0_5_9` | 2026-07-11 | 恢复 `guardian_extension.rb` + `Guardian.prepend`；API/页面校验 `can_use_mechbox?`；metadata 返回 `can_manage_templates` | 待验证：非授权组 403；管理员 `can_manage_templates: true` |
| `phase1_1` | — | 计划：恢复 `calculator_registry` 与 `calculate` API | 待做 |
