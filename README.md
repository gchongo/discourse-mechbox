...
**Phase 0.5.4（增量恢复中）**：论坛稳定运行；`/mechbox` 支持硬刷新；`metadata` 含工具目录；API FeatureGate 已恢复。页面仍为占位内容，工具目录 UI 计划在 Phase 0.5.7 接入。
...
| `phase0_5_4` | 2026-07-11 | 恢复 `feature_gate.rb`，路径为 `lib/discourse_mechbox/feature_gate.rb`；接入 `BaseController`，不使用有 Zeitwerk 风险的 `concerns/` 目录 | `status.mode` 为 `phase0_5_4`；论坛不 502 |
