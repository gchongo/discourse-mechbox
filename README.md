...
**Phase 0.5.6（待生产验证）**：工具目录只读 API 已恢复，不依赖 Guardian 或数据库模型。页面仍为占位内容，工具目录 UI 将在下一步 Phase 0.5.7 接入。
...
| `phase0_5_6` | 2026-07-11 | 恢复无数据库依赖的 `ToolsController`；启用 `/tools` 与 `/tools/:tool_id`；`tools` capability 改为 enabled | 待验证：工具列表含 5 项，`gear_ratio` schema 可读取 |
| `phase0_5_7` | — | 计划：前端 `/mechbox` 渲染工具目录 | 待做 |
