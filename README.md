...
**Phase 0.5.2（增量恢复中）**：论坛稳定运行；侧边栏有 MechBox 入口；`status`/`metadata` API 含 `capabilities` 与 `database_available`。业务代码在 `archived/`，按 [docs/PHASED_ROADMAP.md](docs/PHASED_ROADMAP.md) 小步恢复。502 事故原因见 [docs/BOOT_FAILURES.md](docs/BOOT_FAILURES.md)。
...
| `phase0_5_2` | 2026-07-11 | 恢复 `database_features.rb`；`status`/`metadata` 增加 `database_available` | `database_available` 为 `true`/`false` 均可，论坛不 502 |
| `phase0_5_3` | — | 计划：恢复 `tool_catalog.rb` | 待做 |
| `phase0_5_4` | — | 计划：恢复 `feature_gate.rb` | 待做 |
